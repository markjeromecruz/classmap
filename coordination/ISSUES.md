...
| ISS-03 | infra | P2 | CLOSED | Render `<PlanCard>`, query a11y tree (heading role) | Visible card title is a heading element (`<h*>`) so screen-reader users can jump to it via heading navigation | `components/ui/card.tsx` `CardTitle` is a styled `<div>` (`data-slot="card-title"`). Both `components/portfolio/AppCard.tsx` and `components/classmap/PlanCard.tsx` inherited this. Fix: change CardTitle to render an `<h3>` (or thread an `as`/`level` prop). **A partial fix (A-09):** AppCard redesigned to use a direct `<h2>` (no longer uses CardTitle). **B fix:** PlanCard now renders session title as `<h4>` directly. **C verify:** added regression `getByRole("heading", { level: 4 })` in tests/unit/PlanBoard.test.tsx; 116/116 passing. Root cause in `card.tsx` CardTitle still a `<div>` but no remaining consumers in app code — left for follow-up if needed. CLOSED. |
