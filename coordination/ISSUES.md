...
| ISS-03 | infra | P2 | OPEN | Render `<AppCard>` or `<PlanCard>`, query a11y tree | Visible card title is a heading element (`<h*>`) so screen-reader users can jump to it via heading navigation | `components/ui/card.tsx` `CardTitle` is a styled `<div>` (`data-slot="card-title"`). Both `components/portfolio/AppCard.tsx` (portfolio cards) and `components/classmap/PlanCard.tsx` (per-session cards in the weekly plan) inherit this — neither title is exposed as a heading. Fix: change CardTitle to render an `<h3>` (or thread an `as`/`level` prop and pick the right level per usage — `<h3>` for AppCard, `<h4>` for PlanCard since PlanBoard already uses `<h3>` for day labels). Non-blocking but lowers a11y/SEO. |
