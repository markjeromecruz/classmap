...
|--------|-------|--------|--------------------------------------------------------------------|------------|-------|
| CM-07  | B     | NEEDS_TEST | `/classmap/login` + `/classmap/signup` (mock auth screens)     | A-14, A-15 | Shared `<AuthForm mode>` (client) in `components/classmap/auth/`. Both pages wrap in `<ClassmapShell bare>`. signin → /classmap; signup → /classmap/onboarding. Editorial kicker/dek/font-display, accent-clay primary CTA. Cosmetic OTP (any 6 digits via verifyOtp), cosmetic Google button (surfaces friendly note). All inputs + buttons h-11 (44px touch targets). Selectors: `data-slot="login-form" / "signup-form" / "otp-input" / "google-button" / "auth-error"`, `data-testid="auth-submit"`. |
| CM-08  | B     | IN_PROGRESS | `/classmap/onboarding` — 5-step wizard                        | A-14       | Steps: (1) child name, (2) age + grade, (3) US state (autocomplete), (4) learning style + curriculum approach, (5) priority subjects (multi-select, drag-to-order). `db.createChild` on finish (auto-derives ageBand + avatarColor), redirect `/classmap/today`. shadcn Dialog/Drawer responsive. **5 wizard steps = perfect fan-out candidate (5 parallel subagents).** Selectors: `data-slot="wizard"` + `data-step="N"`. |
...
| TC-02  | C     | IN_PROGRESS | Mock auth flow                                    | CM-07, A-15 | signUp, signIn, signOut, session persists across reload. Cosmetic OTP input present but doesn't block. |
