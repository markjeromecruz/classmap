...
| CM-10  | B     | IN_PROGRESS | `/classmap` shell route — IA redirect logic                   | A-14, A-15 | No session → `/classmap/login`. Session + 0 children → `/classmap/onboarding`. Session + ≥1 child → `/classmap/today`. Renders nothing on the path itself (or a tiny "Loading…" before redirect). |
