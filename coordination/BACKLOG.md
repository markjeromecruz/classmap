...
| KM-03  | B     | NEEDS_TEST | `/kindleminds/rooms/[slug]/[threadId]` thread + replies        | KM-02      | SSG via generateStaticParams over THREADS (10 paths, 5 rooms × 2 each). dynamicParams=false; notFound() if missing OR slug≠roomSlug. Thread body in whitespace-pre-line, replies as ordered list with thin left-rule + kicker author/time. Selectors: `data-slot="kindleminds-thread"[data-thread-id][data-room-slug]`, `thread-title`, `thread-body`, `thread-views`, `thread-reply-count`, `thread-replies[data-count]`, per-item `thread-reply[data-reply-id]` + `thread-reply-body`. |
...
| KM-05  | B     | NEEDS_TEST | Update portfolio landing: KindleMinds → live                   | KM-01, KM-02 | `app/page.tsx`: KindleMinds card now `status="live"` with `href="/kindleminds"`. Highlights rewritten to match the shipped 5-rooms-static-demo reality. |
...
| PT-04  | B     | NEEDS_TEST | Update portfolio landing: Patriarch → live                     | PT-01      | `app/page.tsx`: Patriarch card now `status="live"` with `href="/patriarch"`. Highlights rewritten to reflect shipped landing + today + altar features. |
