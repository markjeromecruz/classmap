...
| CM-09  | B     | DONE       | `/classmap/family` — list profiles + state req panel           | A-14, A-16 | Built via 2-parallel-subagent fan-out. `FamilyList` (client) reads via `useAppState`, renders `ChildCard` grid; `ChildCard` (server) per-child with avatar circle + XP/streak/badges row + style/approach line + collapsible `<details>` state-requirement panel via `getStateRequirement(child.state)`. Add-child + Portfolio-export Links, Logout Button calls signOut + router.push("/classmap/login"). Selectors: `data-slot="family"`, `family-list[data-count]`, `family-empty`, `family-add-child`, `family-actions`, `family-portfolio-link`, `family-logout`, `child-card[data-child-id]`, sub-slots `child-xp`/`child-streak`/`child-badges`/`child-state-req`. |
| CM-10  | B     | DONE_PENDING_A | `/classmap` shell route — IA redirect logic                | A-14, A-15 | Verified by C — tests/unit/classmap-shell-route.test.tsx (6 tests). Loading placeholder + aria-busy/aria-live, all 3 redirect branches (no session→login, session+0 kids→onboarding, session+≥1 kid→today), multi-child also routes to today, effect fires exactly once. |
| CM-11  | B     | DONE_PENDING_A | Mobile bottom nav + side nav + child switcher              | A-17       | Verified by C — tests/unit/classmap-SideNav.test.tsx (18 tests). All 9 nav links in source order; CM-11 touch-target contract (h-11, no h-10) on every link + sign-out; active aria-current=page (exact + child-route); sign-out clears session + router.replace("/classmap/login"). |
...
| TC-04  | C     | DONE   | Family page                                            | CM-09       | Renders all children, avatars present, state requirements panel toggles, add-child reopens wizard. |
