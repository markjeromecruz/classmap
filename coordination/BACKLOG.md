...
| CM-04  | B     | NEEDS_TEST | `/classmap/saved` + `lib/storage.ts` (localStorage)     | CM-02      | Save button on result page; saved list page; delete button. Storage key: `classmap:saved-plans:v1`; entries Zod-validated on read so corrupt rows are dropped. |
